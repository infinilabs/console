...
- feat: add activity-aware access-token refresh for active users, plus a refresh endpoint and absolute `expires_at` metadata so Console sessions can renew in the background instead of forcing logout after exactly two hours
...
- fix: repair legacy DataTools migration/comparison execution by accepting string repeat intervals, refreshing target/source indices before auto-created comparisons run, removing stale diagnostic fields, and hiding unstarted comparison timestamps instead of showing `1970-01-01`
...
- improve: make the Agent one-line install script accept `-u` as a site root, package directory, or full archive URL, and omit `-u` entirely when the default official download source is used
...
- improve: polish the Dev Tools Console page and popup by removing outer scroll jitter, keeping the status bar pinned to the bottom, and making first-load editor typography more stable
