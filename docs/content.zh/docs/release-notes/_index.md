...
- feat: 为活跃用户会话增加 access token 后台自动续期能力，补充 refresh 接口和绝对过期时间 `expires_at` 字段，避免 Console 在固定两小时后直接强制退出
...
- fix: 修复旧版 DataTools 迁移/比对链路：支持字符串格式的重复执行间隔，在自动创建比对前补齐源/目标索引 refresh，移除旧诊断字段，并避免未开始的比对详情显示 `1970-01-01`
...
- improve: 优化 Agent 一键安装脚本的 `-u` 下载地址处理，兼容站点根地址、安装包目录和完整安装包 URL 三种形式，并在使用官方默认下载源时省略 `-u` 让命令更短
...
- improve: 优化 Dev Tools Console 页面与浮层体验：去掉外层滚动抖动，让状态栏始终贴底，并让编辑器首次加载时的字体显示更稳定
