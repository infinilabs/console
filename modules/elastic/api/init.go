...
func init() {
	...
	api.HandleAPIMethod(api.POST, "/elasticsearch/", clusterAPI.RequireSecureTransport(clusterAPI.RequireReplayProtection(clusterAPI.RequirePermission(clusterAPI.HandleCreateClusterAction, enum.PermissionElasticsearchClusterWrite))))
	...
	api.HandleAPIMethod(api.POST, "/elasticsearch/:id/search_template", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleCreateSearchTemplateAction, enum.PermissionElasticsearchClusterWrite)))
	api.HandleAPIMethod(api.PUT, "/elasticsearch/:id/search_template/:template_id", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleUpdateSearchTemplateAction, enum.PermissionElasticsearchClusterWrite)))
	api.HandleAPIMethod(api.DELETE, "/elasticsearch/:id/search_template/:template_id", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleDeleteSearchTemplateAction, enum.PermissionElasticsearchClusterWrite)))
	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/search_template", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleSearchSearchTemplateAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/search_template/:template_id", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleGetSearchTemplateAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/search_template_history/_search", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleSearchSearchTemplateHistoryAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.POST, "/elasticsearch/:id/_render/template", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleRenderTemplateAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.POST, "/elasticsearch/:id/_search/template", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleSearchTemplateAction, enum.PermissionElasticsearchClusterRead)))
	...
	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/search/trace_id", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleTraceIDSearchAction, enum.PermissionElasticsearchMetricRead)))
	...
	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/migration/indices/realtime", clusterAPI.RequireLogin(clusterAPI.GetRealtimeClusterIndices2))
	...
	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/trace_template", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleSearchTraceTemplateAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/trace_template/:template_id", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleGetTraceTemplateAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.POST, "/elasticsearch/:id/trace_template", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleCrateTraceTemplateAction, enum.PermissionElasticsearchClusterWrite)))
	api.HandleAPIMethod(api.PUT, "/elasticsearch/:id/trace_template/:template_id", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleSaveTraceTemplateAction, enum.PermissionElasticsearchClusterWrite)))
	api.HandleAPIMethod(api.DELETE, "/elasticsearch/:id/trace_template/:template_id", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleDeleteTraceTemplateAction, enum.PermissionElasticsearchClusterWrite)))
	...
	api.HandleAPIMethod(api.GET, "/host/_discover", clusterAPI.RequirePermission(clusterAPI.getDiscoverHosts, enum.PermissionElasticsearchNodeRead, enum.PermissionElasticsearchMetricRead))
	...
	api.HandleAPIMethod(api.GET, "/host/:host_id", clusterAPI.RequirePermission(clusterAPI.GetHostInfo, enum.PermissionElasticsearchNodeRead))
	api.HandleAPIMethod(api.PUT, "/host/:host_id", clusterAPI.RequirePermission(clusterAPI.updateHost, enum.PermissionElasticsearchClusterWrite))
	api.HandleAPIMethod(api.GET, "/host/:host_id/info", clusterAPI.RequirePermission(clusterAPI.GetHostOverviewInfo, enum.PermissionElasticsearchNodeRead, enum.PermissionElasticsearchMetricRead))

	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/_ilm/policy", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleGetILMPolicyAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.PUT, "/elasticsearch/:id/_ilm/policy/:policy", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleSaveILMPolicyAction, enum.PermissionElasticsearchClusterWrite)))
	api.HandleAPIMethod(api.DELETE, "/elasticsearch/:id/_ilm/policy/:policy", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleDeleteILMPolicyAction, enum.PermissionElasticsearchClusterWrite)))

	api.HandleAPIMethod(api.GET, "/elasticsearch/:id/_template", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleGetTemplateAction, enum.PermissionElasticsearchClusterRead)))
	api.HandleAPIMethod(api.PUT, "/elasticsearch/:id/_template/:template_name", clusterAPI.RequireClusterPermission(clusterAPI.RequirePermission(clusterAPI.HandleSaveTemplateAction, enum.PermissionElasticsearchClusterWrite)))
