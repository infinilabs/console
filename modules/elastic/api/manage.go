...
func getManagedAgentCredential(conf *elastic.ElasticsearchConfig, previousClusterName string) (*credential.Credential, *model.BasicAuth, bool, error) {
	...
	if err != nil {
		if isCredentialSecretDecodeError(err) {
			if isAutoGeneratedManagedAgentCredential(conf, cred, previousClusterName) {
				conf.AgentCredentialID = ""
				conf.AgentBasicAuth = nil
				return nil, nil, true, nil
			}
			return cred, nil, false, nil
		}
		return nil, nil, false, err
	}
...
func isAutoGeneratedManagedAgentCredential(conf *elastic.ElasticsearchConfig, cred *credential.Credential, previousClusterName string) bool {
	if conf == nil || cred == nil {
		return false
	}
	return isAutoGeneratedClusterCredentialName(cred.Name, conf.Name, conf.ID, clusterCredentialKindAgent) ||
