...
func TestIsAutoGeneratedManagedAgentCredential(t *testing.T) {
	conf := &elastic.ElasticsearchConfig{
		ORMObjectBase: orm.ORMObjectBase{ID: "cluster-1"},
		Name:          "current-cluster",
	}
	cred := &credential.Credential{Name: "previous-cluster (Agent)"}

	if !isAutoGeneratedManagedAgentCredential(conf, cred, "previous-cluster") {
		t.Fatal("expected previous auto-generated agent credential name to be recognized")
	}
}

func TestIsCredentialSecretDecodeError(t *testing.T) {
	if !isCredentialSecretDecodeError(errors.New("cipher: message authentication failed")) {
		t.Fatal("expected aes-gcm auth failure to be treated as credential secret mismatch")
	}
	if isCredentialSecretDecodeError(errors.New("connection refused")) {
		t.Fatal("expected unrelated errors to be ignored")
	}
}
