...
export default class QueryStringInputUI extends Component<Props, State> {
  ...
  private getSuggestions = async () => {
    ...
    const hasQuerySuggestions = this.props.services.data.autocomplete.hasQuerySuggestions(language);
      ...
      if (notifications && docLinks) {
        const toast = notifications.toasts.add({
          ...
          text: toMountPoint(
            <div>
              <p>
                It looks like you're querying on a nested field.
                  You can construct KQL syntax for nested queries in different ways, depending on the results you want.
                  Learn more in our <EuiLink href={docLinks.links.query.kueryQuerySyntax} target="_blank">
                        docs
                      </EuiLink>.
              </p>
          ...
          >
            ...
            >
              <EuiTextArea
                ...
                autoComplete="off"
                spellCheck={false}
                aria-label={'Start typing to search and filter the discover page'}
