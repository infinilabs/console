...
interface StepIndexPatternProps {
  ...
  goToNextStep: (query: string, viewName: string, timestampField?: string) => void;
  initialQuery?: string;
...
export class StepIndexPattern extends Component<StepIndexPatternProps, StepIndexPatternState> {
  ...
  constructor(props: StepIndexPatternProps, context: IndexPatternManagmentContextValue) {
    super(props, context);
    const { indexPatternCreationType, initialQuery, initialViewName } = this.props;
  ...
  queryIndices = async (query: string) => {
    ...
    const newState = results.reduce((prev, item)=>{
      prev.partialMatchedIndices = _.unionBy(prev.partialMatchedIndices,item?.partialMatchedIndices, 'name');
      prev.exactMatchedIndices = _.unionBy(prev.exactMatchedIndices,item?.exactMatchedIndices, 'name');
      return prev;
    }, {
      partialMatchedIndices: [],
      exactMatchedIndices: [],
  ...
  renderHeader({ exactMatchedIndices: indices }: { exactMatchedIndices: MatchedItem[] }) {
    ...
    if (!query || !query.length || query === '.' || query === '..') {
      ...
      const errorMessage = `匹配规则 ${indexPatternName} 不能包含空格或者字符: ${characterList}`;
    ...
    return (
      <Header
        ...
        goToNextStep={(query, viewName) => goToNextStep(query, viewName, canPreselectTimeField(indices))}
