...
> {
  ...
  constructor(props: RouteComponentProps, context: IndexPatternManagmentContextValue) {
    ...
    this.state = {
      ...
      toasts: [],
      indexPatternCreationType: indexPatternManagementStart.creation.getType(type),//context.services.indexPatternManagementStart.creation.getType(type),
  ...
  createIndexPattern = async (timeFieldName: string | undefined, indexPatternId: string) => {
    ...
    try {
      emptyPattern = await data.indexPatterns.createAndSave({//this.context.services.data.indexPatterns.createAndSave({
        id: indexPatternId,
        title: indexPattern,
        viewName: viewName,
        timeFieldName,
      ...
      if (err instanceof DuplicateIndexPatternError) {
        const confirmMessage = `An index pattern with the title '${emptyPattern!.title}' already exists.`;
  ...
  goToTimeFieldStep = (indexPattern: string, viewName: string, selectedTimeField?: string) => {
  ...
  renderContent() {
    const { allIndices, isInitiallyLoadingIndices, step, indexPattern, viewName } = this.state;
