...
import { Position, Range } from "../../entities/core_editor";
...
export class LegacyCoreEditor implements CoreEditor {
  ...
  constructor(private editor: IAceEditor, actions: HTMLElement) {
    ...
    const maxTokenCount = localStorage.getItem("editor:MaxTokenCount");
    if (maxTokenCount) {
      this.setMaxTokenCount(maxTokenCount);
    }
  ...
  setMaxTokenCount(count: any) {
    const session = this.editor.getSession();
    session
      .getMode()
      .getTokenizer()
      ?.$setMaxTokenCount(count);
  }
    ...
    return new Promise<void>((resolve) => {
      ...
      const check = () => {
        ...
        if (session.bgTokenizer) {
          ...
          if (
            ((session.bgTokenizer as unknown) as { running: boolean }).running
          ) {
  ...
  isCompleterActive() {
    return Boolean(
      ((this.editor as unknown) as { completer: { activated: unknown } })
        .completer &&
        ((this.editor as unknown) as { completer: { activated: unknown } })
          .completer.activated
    );
