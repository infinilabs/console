...
import { default as fetchRequest } from "@/utils/request";
...
export class Fetch {
  ...
  public fetch: HttpHandler = async <TResponseBody>(
    ...
    return new Promise<TResponseBody | HttpResponse<TResponseBody>>(
  ...
  private async fetchResponse(
    ...
    try {
      ...
      response = (await fetchRequest(request.url, fetchOptions, true)) as any;
    } catch (err) {
      console.warn(
        new HttpFetchError(err.message, err.name ?? "Error", request)
      );
      response = {};
    }
...
const validateFetchArguments = (
  ...
  const invalidHeaders = Object.keys(
    fullOptions.headers ?? {}
  ).filter((headerName) => headerName.startsWith("kbn-"));
