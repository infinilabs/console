...
fieldFormatsMock.getInstance = jest
  .fn()
  .mockImplementation(() => new MockFieldFormatter()) as any;
...
describe("IndexPattern", () => {
  ...
  describe("getComputedFields", () => {
    ...
    test("should request date field doc values in date_time format", () => {
      const { docvalueFields } = indexPattern.getComputedFields();
      const timestampField = docvalueFields.find(
  ...
  describe("add and remove scripted fields", () => {
    test("should append the scripted field", async () => {
      ...
      expect(
        (indexPattern.fields.getByName(scriptedField.name) as IndexPatternField)
          .name
      ).toEqual(scriptedField.name);
    });
    ...
    test("should remove scripted field, by name", async () => {
      ...
      expect(indexPattern.fields.getByName(scriptedField.name)).toEqual(
        undefined
      );
    });
  ...
  describe("toSpec", () => {
    ...
    test("can restore from spec", async () => {
      ...
      expect(
        restoredPattern.fieldFormatMap.bytes instanceof MockFieldFormatter
      ).toEqual(true);
    });
  });
});
