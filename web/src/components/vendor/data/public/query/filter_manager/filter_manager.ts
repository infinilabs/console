...
export class FilterManager {
  ...
  private mergeIncomingFilters(
    partitionedFilters: PartitionedFilters
  ): Filter[] {
  ...
  private static mergeFilters(
    ...
    return uniqFilters(
      appFilters.reverse().concat(globalFilters.reverse())
    ).reverse();
  }
  ...
  private handleStateUpdate(newFilters: Filter[]) {
    ...
    const filtersUpdated = !compareFilters(
      this.filters,
      newFilters,
      COMPARE_ALL_OPTIONS
    );
    const updatedOnlyDisabledFilters = onlyDisabledFiltersChanged(
      newFilters,
      this.filters
    );
  ...
  public addFilters(
    ...
    pinFilterStatus: boolean = true //this.uiSettings.get(UI_SETTINGS.FILTERS_PINNED_BY_DEFAULT)
  ) {
  ...
  public removeFilter(filter: Filter) {
    const filterIndex = _.findIndex(this.filters, (item) => {
      return (
        _.isEqual(item.meta, filter.meta) && _.isEqual(item.query, filter.query)
      );
    });
