...
import { BaseStateContainer } from "../../../../utils/public";
...
export const connectToQueryState = <S extends QueryState>(
  ...
  const subs: Subscription[] = [
        ...
        map(({ changes }) => {
          ...
          if (syncConfig.filters) {
            if (syncConfig.filters === true && changes.filters) {
              ...
              syncConfig.filters === FilterStateStore.APP_STATE &&
              changes.appFilters
            ) {
    ...
    stateContainer.state$.subscribe((state) => {
      ...
      if (syncConfig.time) {
        const time = validateTimeRange(state.time)
          ? state.time
          : timefilter.getTimeDefaults();
      ...
      if (syncConfig.refreshInterval) {
        const refreshInterval =
          state.refreshInterval || timefilter.getRefreshIntervalDefaults();
