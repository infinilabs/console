...
export default class QueryStringInputUI extends Component<Props, State> {
  ...
  private fetchIndexPatterns = async () => {
    ...
    const objectPatternsFromStrings = (await fetchIndexPatterns(
      ...
      this.props.services.savedObjects!.client,
      stringPatterns,
  ...
  private getSuggestions = async () => {
    ...
    const recentSearchSuggestions = this.getRecentSearchSuggestions(
      queryString
    );
    const hasQuerySuggestions = this.props.services.data.autocomplete.hasQuerySuggestions(
      language
    );
    ...
    try {
      ...
      const suggestions =
        (await this.props.services.data.autocomplete.getQuerySuggestions({
          language,
          indexPatterns,
          query: queryString,
          selectionStart,
  ...
  private getRecentSearchSuggestions = (query: string) => {
    ...
    const matchingRecentSearches = recentSearches.filter((recentQuery) => {
      const recentQueryString =
        typeof recentQuery === "object" ? toUser(recentQuery) : recentQuery;
  ...
  private onKeyUp = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (
      [KEY_CODES.LEFT, KEY_CODES.RIGHT, KEY_CODES.HOME, KEY_CODES.END].includes(
        event.keyCode
      )
    ) {
      ...
      switch (event.keyCode) {
        case KEY_CODES.DOWN:
          if (isSuggestionsVisible && index !== null) {
            ...
            (isSuggestionsVisible && index == null) ||
            this.getQueryString() === ""
          ) {
            event.preventDefault();
    ...
    if (
      ...
      const onKQLNestedQuerySyntaxInfoOptOut = (toast: Toast) => {
        ...
        this.services.storage.set(
          "kibana.KQLNestedQuerySyntaxInfoOptOut",
          true
        );
      ...
      if (notifications && docLinks) {
        const toast = notifications.toasts.add({
          title: "KQL nested query syntax",
          text: toMountPoint(
            <div>
              <p>
                It looks like you're querying on a nested field. You can
                construct KQL syntax for nested queries in different ways,
                depending on the results you want. Learn more in our{" "}
                <EuiLink
  ...
  public componentDidUpdate(prevProps: Props) {
    ...
    if (
      ...
      if (
        ...
        document.activeElement.id === this.textareaId
      ) {
        this.handleAutoHeight();
      } else {
        this.handleRemoveHeight();
  ...
  handleAutoHeight = () => {
    if (this.inputRef !== null && document.activeElement === this.inputRef) {
      this.inputRef.style.setProperty(
        "height",
        `${this.inputRef.scrollHeight}px`,
        "important"
      );
    }
    this.handleListUpdate();
      ...
      <div className={className}>
        ...
        <EuiOutsideClickDetector onOutsideClick={this.onOutsideClick}>
          <div
            ...
            aria-label={"Search and filter the discover page"}
            aria-haspopup="true"
          ...
          >
            ...
            >
              <EuiTextArea
                ...
                id={this.textareaId}
                autoFocus={
                  this.props.onChangeQueryInputFocus
                    ? false
                ...
                spellCheck={false}
                aria-label={
                  "Start typing to search and filter the discover page"
                }
