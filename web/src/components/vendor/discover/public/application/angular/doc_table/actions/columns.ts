...
export function getStateColumnActions({
  ...
  function onAddColumn(columnName) {
    setAppState((st) => {
      const columns = addColumn(st.columns || [], columnName, useNewFieldsApi);
      const defaultOrder = "desc";
      const sort =
        columnName === "_score" && !st.sort?.length
          ? [["_score", defaultOrder]]
          : st.sort;
      return { columns, sort };
    });
  ...
  function onRemoveColumn(columnName) {
    setAppState((st) => {
      ...
      const sort =
        st.sort && st.sort.length
          ? st.sort.filter((subArr) => subArr[0] !== columnName)
          : [];
      return { columns, sort };
    });
  }
