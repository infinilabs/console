...
function getFieldValueCounts(params) {
  ...
  if (
    ...
    return {
      error:  'Analysis is not available for geo fields.'
    };
  }
  ...
  try {
    ...
    if (params.hits.length - missing === 0) {
      return {
        error: 
            `This field is present in your Elasticsearch mapping but not in the ${params.hits.length} documents shown in the doc table. You may still be able to visualize or search on it.`,
      };
    }
...
function _groupValues(allValues, params) {
  ...
  allValues.forEach(function (value) {
    if (_.isObject(value) && !Array.isArray(value)) {
      throw new Error(
     'Analysis is not available for object fields.',
      );
    }
