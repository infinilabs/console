...
import {
  UrlForwardingSetup,
  UrlForwardingStart,
} from "src/plugins/url_forwarding/public";
...
import { createKbnUrlTracker } from "../../utils/public";
import { DEFAULT_APP_CATEGORIES } from "../../../core/public";
import { UrlGeneratorState } from "../../share/public";
import {
  DocViewInput,
  DocViewInputFn,
} from "./application/doc_views/doc_views_types";
...
export interface DiscoverStart {
   ...
   * ```
   */
  readonly urlGenerator:
    | undefined
    | UrlGeneratorContract<"DISCOVER_APP_URL_GENERATOR">;
}
...
export class DiscoverPlugin
  ...
  public initializeServices?: () => Promise<{
    core: CoreStart;
  ...
  setup(
    ...
    core.application.register({
      ...
      mount: async (params: AppMountParameters) => {
        ...
        if (!this.initializeInnerAngular) {
          throw Error(
            "Discover plugin method initializeInnerAngular is undefined"
          );
        }
    ...
    return {
      docViews: {
        addDocView: this.docViewsRegistry.addDocView.bind(
  ...
  private registerEmbeddable(
    ...
    const getStartServices = async () => {
      ...
      return {
        ...
        isEditable: () =>
          coreStart.application.capabilities.discover.save as boolean,
      };
    };
  ...
  private getEmbeddableInjector = async () => {
    if (!this.embeddableInjector) {
      if (!this.initializeServices) {
        throw Error(
          "Discover plugin getEmbeddableInjector:  initializeServices is undefined"
        );
      }
      ...
      this.embeddableInjector = angular.bootstrap(mountpoint, [
