...
> {
  ...
  renderPagination() {
    ...
    const button = (
      ...
      >
        {formatMessage({ id: "explore.table.rows_of_page" }) + `: ${perPage}`}
      </EuiButtonEmpty>
    );
  ...
  render() {
    ...
    const queryWithoutWildcard = query.endsWith("*")
      ? query.substr(0, query.length - 1)
      : query;

    const paginatedIndices = indices.slice(
      this.pager.firstItemIndex,
      this.pager.lastItemIndex + 1
    );
