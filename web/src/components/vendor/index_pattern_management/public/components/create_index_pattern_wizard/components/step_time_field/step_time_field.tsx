...
> {
  ...
  constructor(props: StepTimeFieldProps) {
    super(props);
    this.state.indexPatternType =
      props.indexPatternCreationType.getIndexPatternType() || "";
  ...
  fetchTimeFields = async () => {
    ...
    const fields = await ensureMinimumTime(
      ...
      useGlobalContext().data.indexPatterns.getFieldsForWildcard({
        pattern,
  ...
  onTimeFieldChanged = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    this.setState({
      selectedTimeField: timeField
        ? (timeField as TimeFieldConfig).fieldName
        : undefined,
  ...
  formatErrorMessage(message: string) {
    // `createIndexPattern` throws "Conflict" when view ID already exists.
    return message === "Conflict" ? "匹配规则已经存在。" : message;
  }
  ...
  render() {
      ...
      return (
        ...
        >
          <EuiFlexItem grow={false}>
            <EuiTitle size="s">
              <h3 className="eui-textCenter">Creating View…</h3>
            </EuiTitle>
          </EuiFlexItem>
    ...
    const error = this.state.error ? (
      <>
        <EuiCallOut title={"Error"} color="danger" iconType="cross">
