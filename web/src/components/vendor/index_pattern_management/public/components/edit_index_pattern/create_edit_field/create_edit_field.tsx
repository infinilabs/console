...
const newFieldPlaceholder = "New Scripted Field";
...
export const CreateEditField = withRouter(
  ({ indexPattern, mode, fieldName, history }: CreateEditFieldProps) => {
    ...
    if (mode === "edit" && !spec) {
      ...
      //   {
      //     defaultMessage:
      //       "'{indexPatternTitle}' view doesn't have a scripted field called '{fieldName}'",
    ...
    const redirectAway = () => {
      history.push(
        `${url}#/?_a=(tab:${
          spec?.scripted ? TAB_SCRIPTED_FIELDS : TAB_INDEXED_FIELDS
        })`
      );
    };
