...
export function createEditIndexPatternPageStateContainer({
  ...
  const initialStateFromUrl = kbnUrlStateStorage.get<IEditIndexPatternState>(
    stateStorageKey
  );
  const stateContainer = createStateContainer(
    ...
    {
      setTab: (state: IEditIndexPatternState) => (tab: string) => ({
        ...state,
        tab,
      }),
    },
  ...
  return {
    ...
    setCurrentTab: (newTab: string) =>
      stateContainer.transitions.setTab(newTab),
