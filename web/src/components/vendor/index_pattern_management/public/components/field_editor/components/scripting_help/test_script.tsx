...
export class TestScript extends Component<TestScriptProps, TestScriptState> {
  ...
  previewScript = async (searchContext?: { query?: Query | undefined }) => {
    ...
    let query;
    const { uiSettings, http } = useGlobalContext();
    if (searchContext) {
      const esQueryConfigs = esQuery.getEsQueryConfig(uiSettings); //this.context.services.uiSettings);
    ...
    const scriptResponse = await executeScript({
      ...
      query,
      additionalFields: this.state.additionalFields.map(
        (option: AdditionalField) => option.value
      ),
      http: http, //this.context.services.http,
    });
      ...
      <Fragment>
        <EuiFormRow label={"Additional fields"} fullWidth>
          <EuiComboBox
            ...
            onChange={(selected) =>
              this.onAdditionalFieldsChange(selected as AdditionalField[])
            }
    ...
    return (
      <Fragment>
        ...
        <EuiText>
          ...
          <p>
            Run your script to preview the first 10 results. You can also select
            some additional fields to include in your results to gain more
            context or add a query to filter on specific documents.
          </p>
        </EuiText>
        <EuiSpacer />
        {this.renderToolbar()}
