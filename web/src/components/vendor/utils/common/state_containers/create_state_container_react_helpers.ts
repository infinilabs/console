 ...
 * @public
 */
export const useContainerSelector = <
...
>(
  ...
  useLayoutEffect(() => {
    const subscription = state$.subscribe(
      (currentState: UnboxState<Container>) => {
 ...
 * Refer to {@link https://github.com/elastic/kibana/blob/master/src/plugins/utils/docs/state_containers/react.md | guide} for details
 * @public
 */
...
>() => {
  ...
  const useTransitions: () => Container["transitions"] = () =>
    useContainer().transitions;
  ...
  const useSelector = <Result>(
    ...
    const container = useContainer();
    return useContainerSelector<Container, Result>(
      container,
      selector,
      comparator
    );
  ...
  const connect: Connect<UnboxState<Container>> = (mapStateToProp) => (
    component
  ) => (props) =>
