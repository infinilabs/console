 ...
 *
 * Go {@link https://github.com/elastic/kibana/tree/master/src/plugins/utils/docs/state_sync | here} for a complete guide and examples.
 *
 * @example
 *
  ...
  return {
    ...
    start: () => {
      if (subscriptions.length > 0) {
        throw new Error(
          "syncState: can't start syncing state, when syncing is in progress"
        );
      }
      subscriptions.push(
        onStateChange$.subscribe(),
        onStorageChange$.subscribe()
      );
    },
  };
}
