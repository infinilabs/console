...
import Logging from "../../../../../Migration/Migration/Detail/MigrateIndices/Detail/Logging";
...
export default ({ taskId, record, refreshFlag, logInfo, repeat={} }) => {
  ...
  const {
    ...
    `/comparison/data/${taskId}/info/${sourceIndex}:${sourceDocType}`,
    {},
    [taskId, sourceIndex, sourceDocType, refreshFlag]
  );

  const { partitions, repeating } = value || {};
    ...
    <Spin spinning={!refreshFlag && loading}>
      ...
      <TabPane tab={formatMessage({id:"migration.title.progress"})} key="progress">
        <Partitions
            ...
            onRefresh={run}
            logInfo={logInfo}
            nextRunTime={repeat.next_run_time}
          />
      </TabPane>
