...
import Info from "../../../Migration/Migration/Detail/Info";
...
export default (props) => {
  ...
  const { run, loading, value } = useFetch(`/comparison/data/${id}/info`, {}, [
    id,
  ]);

  const [refreshIntervalFlag, setRefreshIntervalFlag] = useState();

  const config = value && (value.config || JSON.parse(value.config_string) ) || {};
  const metadata = value && value.metadata;
  let autoName = '';
  if(!config.name && config.cluster?.source){
    config.indicesCount = config.indices.length;
    autoName = generateName(config)
  }
  ...
  const tags = metadata?.labels?.tags || [];
