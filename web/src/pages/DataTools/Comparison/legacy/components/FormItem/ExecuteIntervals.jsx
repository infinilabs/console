  ...
  return (
    <div className={styles.executeIntervalItem}>
      <div key={index} className={styles.interval}>
        ...
        <TimePicker
          placeholder={formatMessage({ id: "migration.placeholder.execute_start_time" })}
        ...
        <span style={{ margin: "0 6px" }}>
          {formatMessage({ id: "migration.label.to" })}
        </span>
        <TimePicker
          placeholder={formatMessage({ id: "migration.placeholder.execute_end_time" })}
...
export default ({ record, form }) => {
  ...
  return (
    <Form.Item label={formatMessage({ id: "migration.label.time_window" })}>
        ...
        rules: [
          {
            required: true,
             message: formatMessage({
               id: "migration.validation.execute_start_time",
               defaultMessage: "Please select execute start time!",
             }),
          },
          {
            validator: (rule, value, callback) => {
              if (!value || value.length === 0) {
                ...
                if (isEmpty) {
                   callback(
                     formatMessage({
                       id: "migration.validation.execute_end_time",
                       defaultMessage: "Please select execute end time!",
                     })
                   );
                }
              }
