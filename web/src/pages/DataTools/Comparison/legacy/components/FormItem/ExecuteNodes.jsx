...
export default ({ record, form, onChange }) => {
  ...
  const [queryParams, setQueryParams] = useState({ size: 10, from: 0 });
  ...
  return (
    <Form.Item label={formatMessage({ id: "migration.setting.workers" })}>
      {getFieldDecorator("settings.execution.nodes.permit", {
        initialValue: permit ? permit?.map((item) => item.id) : [],
        rules: [
          {
            required: true,
            message: formatMessage({ id: "migration.validation.execute_nodes" }),
          },
        ],
      })(
        <DropdownList 
        ...
        allowClear
        searchKey="name"
        placeholder={formatMessage({ id: "migration.placeholder.search_nodes" })}
        onSearchChange={onSearch}
        sorter={sorter}
        onSorterChange={onSorterChange}
        sorterOptions={[
            { label: formatMessage({ id: "migration.sorter.worker_name" }), key: "name" },
        ]}
        pagination={{
