...
export default ({
  ...
  const retryFailedSubTasks = async () => {
    if (retryableIndices.length === 0) {
      return message.info(
        formatMessage({
          id: "migration.retry_failed.empty",
    ...
    try {
      ...
      for (const item of retryableIndices) {
        const detail = await request(
          `/migration/data/${taskId}/info/${item.sourceIndex}:${item.sourceDocType}`
        );
      ...
      const results = await Promise.allSettled(
        uniqueTaskIDs.map((id) =>
          request(`/migration/data/${id}/_start`, {
            method: "POST",
          })
        )
      );
      ...
      if (failedCount > 0) {
        message.error(
          formatMessage({
            id: "migration.retry_failed.partial",
        ...
        message.success(
          formatMessage({
            id: "migration.retry_failed.success",
        ...
        return <div style={{display:"flex", gap: 20, alignItems:"center",}}>
          <div style={{width:260}}>
            ...
            <div style={{display:"flex", gap:8, alignItems:"center", justifyContent:"center",}}>
              ...
              <SegmentedProgress
                percent={record.exported_percent}
                color={record.error_partitions > 0 ? "#F35F5A" : "#448EF7"}
                height={8}
                segments={20}
              />
              </div>
        ...
        return <div style={{width:260}}>
          ...
          <div style={{display:"flex", gap:8, alignItems:"center", justifyContent:"center"}}>
            ...
            <SegmentedProgress
              ...
              color={record.error_partitions > 0 ? "#F35F5A" : "#6CCE79"}
              height={8}
              segments={20}
            />
            </div>
    ...
    <div className={styles.migrationList}>
      <div className={styles.header}>
        ...
        <div className={styles.actions}>
          {hasAuthority("data_tools.migration:all") && retryableIndices.length > 0 ? (
            <Popconfirm
              title={formatMessage({
                id: "migration.retry_failed.confirm",
            ...
            >
              <Button loading={retryingFailed}>
                {formatMessage({
                  id: "migration.retry_failed.button",
