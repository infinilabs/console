...
export default Form.create({ name: "form_new" })((props) => {
  ...
  const steps = [
    ...
    {
      title: formatMessage({ id: "migration.step.initialize_configuration" }),
    },
    {
      title: formatMessage({ id: "migration.step.migrate_setting" }),
    },
    {
  ...
  const next = async () => {
    if (current === 0) {
      ...
      if(hasErr){
        confirm({
          title: formatMessage({ id: "migration.warning.init_failed_title" }),
          content: formatMessage({ id: "migration.warning.init_failed_content" }),
          onOk() {
