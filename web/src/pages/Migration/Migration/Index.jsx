...
const Index = (props) => {
  ...
  const formatData = (data) => {
    ...
    return data.map((item) => {
      ...
      const config = item.config || JSON.parse(item.config_string) || {};
      const indices = config.indices || [];
      const indicesCount = indices.length;
      const labels = metadata?.labels || {};
      const error_partitions = labels.error_partitions;
      const documents = repeat.is_repeat
        ? labels.source_total_docs
        : indices.reduce((a, b) => a + (b.source?.docs || 0), 0);
      ...
      let percent =
        status === "init"
          ...
          : labels.source_total_docs
          ? Math.round(
              (labels.target_total_docs / labels.source_total_docs) * 100
            )
          : 0;
      if (labels.target_total_docs === 0 && status == "complete") {
        percent = 100;
      }
      ...
      return {
        ...
        name: config.name || "",
        indexDocs: labels.target_total_docs,
      };
    });
  };
  ...
  const onDeleteClick = useCallback(async (id) => {
    const res = await request(`/migration/data/${id}`, {
      method: "DELETE",
    });
  ...
  const onStatusClick = useCallback(async (id, status) => {
    const res = await request(`/migration/data/${id}/_status`, {
      method: "POST",
      body: { status: status },
    });
  ...
  const onResume = async (id) => {
    const res = await request(`/migration/data/${id}/_resume`, {
      method: "POST",
    });
    if (res && res.success) {
      message.success(
  ...
  const onPause = async (id) => {
    const res = await request(`/migration/data/${id}/_pause`, {
      method: "POST",
    });
    if (res && res.success) {
      message.success(
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, record) => {
        ...
        {formatMessage({ id: "migration.label.total_docs" })}: {formatNumber(record.documents)}<br/>
        {formatMessage({ id: "migration.label.written_docs" })}: {formatNumber(record.indexDocs)}<br/>
        ...
        return <div style={{width:80}}>
          <Tooltip title={progressTitle}>
            <Progress showInfo={false} strokeWidth={16} strokeColor={strokeColor} percent={value} status={status || "active"} />
          </Tooltip>
        </div>;
      },
      width: 120,
    },
    {
      ...
      render: (value, record) => {
        ...
        if(value === 0 && record.status == "init" || record.status == "ready") {  
            runningState = 0;
        }
        ...
        return <div className="td-status">
          <Tag style={{border: "none", ...diffStyle }}>
            ...
            {runningState === 0 && record.repeat.is_repeat ? formatMessage({ id: "migration.label.pending" }) : null}
            {value === 0 && !record.repeat.is_repeat && (record.status == "init" || record.status == "ready" || record.status == "pending_stop") ? formatMessage({ id: "migration.label.pending" }) : null}
            {value === 0 && !record.repeat.is_repeat && record.status != "init" && record.status != "ready" && record.status != "pending_stop" ? formatMessage({ id: "migration.label.stopped" }) : null}
          </Tag>
    ...
    },
    {
      ...
      render: (text, record) => {
        const actions = [];
        const hasPrivilege = hasAuthority("data_tools.migration:all");
        ...
        if (!isRepeat) {
          if (hasPrivilege && ["stopped", "error"].includes(record.status)) {
            actions.push(
              <Popconfirm
        ...
        actions.push(
          <Link key="detail" to={`/data_tools/migration/${record.id}/detail`}>
        ...
        >
          ...
          >
            ...
            {hasAuthority("data_tools.migration:all") ? (
              <Button
                type="primary"
                onClick={() => router.push(`/data_tools/migration/new`)}
              >
