...
export const formatValue = (data, valueField, pos = 2) => {
    if (!data || !valueField) return [];
    return data.map((item) => ({
        ...item,
        [valueField]: Number.isInteger(item[valueField]) ? item[valueField] : formatFloat(item[valueField], pos)
...
export const findDataMaxUnit = (data, valueField) => {
    if (!data || data.length === 0 || !valueField) return ''
    const max = Math.max(...data.map((item) => item[valueField]));
    return findBytesUnit(max)
}

export const autoFormatUnitForBytes = (data, valueField) => {
    if (!data || data.length === 0 || !valueField) return {
        data: [],
        unit: UNIT_BYTES
    }
    const unit = findDataMaxUnit(data, valueField);
    return {
        data: formatValue(data.map((item) => ({
            ...item,
            [valueField]: formatUnitToUnit(item[valueField], UNIT_BYTES, unit)
        })), valueField, 2),
        unit 
    }
}

export const formatPercentData = (data, valueField) => {
    return {
        data: formatValue(data.map((item) => ({
            ...item,
...
export const formatNumberData = (data, valueField) => {
    if (!data || data.length === 0 || !valueField) return {
        data: [],
    }
    const max = Math.max(...data.map((item) => item[valueField]));
    ...
    return {
        data: formatValue(data.map((item) => ({
            ...item,
            [valueField]: item[valueField] / maxNumberUnit.factor
        })), valueField, 2),
